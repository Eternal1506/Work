...
def create_interactive_plotter(history_dir="simulation_history"):
    ...
    # Create a grid: 1 row for plots, 1 for the slider. Left side for plots, right for widgets.
    # Changed width_ratios from [4, 1] to [3, 1] to give more space to the right column.
    gs = fig.add_gridspec(2, 2, width_ratios=[7, 6], height_ratios=[20, 1])

    ax3d = fig.add_subplot(gs[0, 0], projection='3d')
    # Create a secondary gridspec for the 2D plot and its checkboxes
    ...
    ax_check3d = fig.add_subplot(gs[1, 1])

    fig.subplots_adjust(wspace=0.4) # Add space between plot columns
    ...
    # This dictionary will hold the active quiver objects for easy removal
    active_quivers = {}
    ...
    ax2d.set_title("Physical Quantities")
    ax2d.grid(True)
    ...
    def update(val):
        ...
        # --- FIX: Remove old quivers before drawing new ones ---
        ...
        quiver_length_scale = plot_range * 0.05 # Make arrow length 5% of plot range

        if status3d[0] and 'f' in history: # Show Forces
            f_data = history['f'][frame_idx]
            # Normalize each vector and scale it for consistent arrow size
