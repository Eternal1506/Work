...
PARAMS = {
    # --- SCENARIO SETUP ---
    "scenario": "flagellar_wave", # Options: "twist_helix", "gravity_fall", "flagellar_wave"
    ...
    "straight_rod_orientation_axis": 'x',
    "xi_pert": 1e-5,

    # --- GRAVITY PARAMETERS ---
    "gravity_on": False,
    "rod_density_vol": 2.0e-12, "rod_radius": 0.1,
    "gravity_vector": np.array([0.0, 0.0, -9.8e6]),
...
if PARAMS["scenario"] == "twist_helix":
    PARAMS.update({ "M": 76, "ds": 0.0785, "total_time": 0.005, "Omega1": 1.3,
                    "Omega3": np.pi / 2.0, "gravity_on": False, "animation_steps_skip": 100,
                    "straight_rod_orientation_axis": 'z' })
elif PARAMS["scenario"] == "gravity_fall":
    PARAMS.update({ "M": 40, "ds": 0.2, "total_time": 0.1, "dt": 5.0e-6,
                    "gravity_on": True, "animation_steps_skip": 400 })
...
def get_rotation_matrix_sqrt(R_mat):
    try:
        ...
        return Rotation.from_matrix(R_mat).as_rotvec(approximated=True) * 0.5
...
class KirchhoffRod:
    def __init__(self, params):
        ...
        if self.p["initial_shape"]=="straight":
            xi=self.p["xi_pert"]; orient_axis=self.p.get("straight_rod_orientation_axis",'z')
            if orient_axis=='x': self.X[:,0]=self.s_vals;self.D3[:,0]=1.;self.D1[:,1]=1.;self.D2[:,2]=1.
            elif orient_axis=='y': self.X[:,1]=self.s_vals;self.D3[:,1]=1.;self.D1[:,2]=1.;self.D2[:,0]=1.
            else: self.X[:,2]=self.s_vals;self.D3[:,2]=1.;self.D1[:,0]=1.;self.D2[:,1]=1.
        ...
        for i in range(self.M):
            ...
            if np.linalg.norm(d2_ortho)>1e-9: d2_ortho/=np.linalg.norm(d2_ortho)
                ...
                temp_vec=np.array([0.,1.,0.]) if np.abs(d1[0])<.9 else np.array([0.,0.,1.])
    ...
    def update_intrinsic_curvature(self):
        if self.p["scenario"]=="flagellar_wave":
            k=self.p["k_wave"];b=self.p["b_amp"];sigma=self.p["sigma_freq"]
    ...
    def compute_internal_forces_and_moments(self):
        ...
        for k in range(self.M-1):
            ...
            omega_half=(self.Omega[k]+self.Omega[k+1])/2.
    ...
    def compute_fluid_forces_on_rod(self,F_half,N_half):
        ...
        for k in range(self.M):
            ...
            n_on_fluid[k]=dN_ds+.5*cross_term
...
if __name__ == '__main__':
    ...
    if history_X:
        ...
        plot_range=max_range*.6 if max_range>0 else PARAMS["L_eff"]
