...
PARAMS = {
    ...
    # --- Flow snapshot saving ---
    "save_flow_snapshots": False,
    "flow_snapshot_steps_skip": 5000,
    "flow_snapshot_plane": "yz",  # "xy", "xz", "yz"
    "flow_snapshot_n": 40,
    "flow_snapshot_xlim": (-2.0, 2.0),
    "flow_snapshot_ylim": (-2.0, 2.0),
    "flow_snapshot_z": 0.0,
    "flow_snapshot_y": 0.0,
    "flow_snapshot_x": 0.0,
...
if __name__ == '__main__':
    ...
    for step in range(num_steps):
        ...
        # --- Save flow snapshots every N steps ---
        if (PARAMS["save_flow_snapshots"] 
            and step % PARAMS["flow_snapshot_steps_skip"] == 0):

            g0_fil = f_on_rod * rod.ds
            m0_fil = n_on_rod * rod.ds
            if rod.has_head:
                g0_head, m0_head = rod.compute_head_force_and_torque(f_on_rod, n_on_rod)
                X_all = np.vstack([rod.X, rod.X_head.reshape(1, 3)])
                g0_all = np.vstack([g0_fil, g0_head.reshape(1, 3)])
                m0_all = np.vstack([m0_fil, m0_head.reshape(1, 3)])
                eps_all = np.empty(rod.M + 1)
                eps_all[:rod.M] = rod.epsilon_reg
            ...
            nx, ny = PARAMS["flow_snapshot_n"], PARAMS["flow_snapshot_n"]
            xs = np.linspace(*PARAMS["flow_snapshot_xlim"], nx)
            ys = np.linspace(*PARAMS["flow_snapshot_ylim"], ny)
            Xg, Yg = np.meshgrid(xs, ys)

            if PARAMS["flow_snapshot_plane"] == "xy":
                pts = np.column_stack([Xg.ravel(), Yg.ravel(), np.full(Xg.size, PARAMS["flow_snapshot_z"])])
            elif PARAMS["flow_snapshot_plane"] == "xz":
                pts = np.column_stack([Xg.ravel(), np.full(Xg.size, PARAMS["flow_snapshot_y"]), Yg.ravel()])
            elif PARAMS["flow_snapshot_plane"] == "yz":
                pts = np.column_stack([np.full(Xg.size, PARAMS["flow_snapshot_x"]), Xg.ravel(), Yg.ravel()])
            else:
                raise ValueError("Invalid flow_snapshot_plane")

            us = evaluate_velocity_field(pts, X_all, g0_all, m0_all, eps_all, rod.mu)
            Ux = us[:, 0].reshape(Xg.shape)
            Uy = us[:, 1].reshape(Xg.shape)
            speed = np.sqrt(Ux**2 + Uy**2)

            plt.figure(figsize=(7, 6))
            plt.streamplot(xs, ys, Ux, Uy, density=1.5)
            plt.imshow(speed, origin='lower', extent=(*PARAMS["flow_snapshot_xlim"], *PARAMS["flow_snapshot_ylim"]), alpha=0.6)
            plt.scatter(X_all[:, 0], X_all[:, 1], c='k', s=8)
            plt.colorbar(label='speed (um/s)')
            plt.title(f'Flow snapshot {PARAMS["flow_snapshot_plane"]} step {step}')
            os.makedirs("flow_snapshots", exist_ok=True)
            plt.savefig(f"flow_snapshots/flow_snapshot_{PARAMS["flow_snapshot_plane"]}_step{step:06d}.png", dpi=150)
            plt.close()
